...
drop view if exists new_user;
create view new_user as
select  firstname, lastname, gender, birthday, hometown
from users
join profiles
on users.id= profiles.user_id
where birthday > '2003.04.28';
...
select 
	...
	count(from_user_id),
	dense_rank()
	over(order by count(from_user_id) desc) as 'The best'
from users u
join messages m
...
drop view if exists new_masseges;
create view new_masseges as
select 
	*,
    lag(created_at) over(order by created_at) as lag_created_at    
from messages;

select *, timestampdiff(second, lag_created_at, created_at) as 'Interval'
from new_masseges;
